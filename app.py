    ...
    <script>
        ...
        const sortable = new Sortable(listElement, {{
            ...
            onEnd: function() {{
                ...
                // CRITICAL: This sends the data back to Streamlit
        ...
        // Initial send so the list isn't empty on first load
...
with tabs[0]:
    ...
    # Get order from the unified JS component
    ...
    if st.button("LOCK IN PROPHECY", type="primary", use_container_width=True):
        ...
        # Fix for the TypeError: ensure current_order is a valid list of strings
            ...
            try:
                # Same write logic that worked previously
                new_entry = pd.DataFrame([{
                    "Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"),
                    "Oracle Name": oracle_name,
                    "Rankings": ",".join(map(str, current_order))
                }])
...
# --- LEADERBOARD & MATRIX LOGIC (RESTORED) ---
with tabs[1]:
    if not subs_df.empty and not results_df.empty and "Rank" in results_df.columns:
        ...
        st.info("Leaderboard will populate once results and submissions are synced.")
...
with tabs[3]:
    ...
    st.markdown("""
    **Golf Scoring:** Lowest penalty wins.  
    **Formula:** |Predicted Rank - Official Rank| × Multiplier  
    **Multipliers:** #1 (4x), #2 (3x), #3-4 (2x), others (1x).
    """)
